...
When the parser is in =MODE_ORG=, it is only searching for
...
#+END_SRC
***** TODO Header Parsing
One of the newer things that the Worgle parser looks for
are header/section data. This information is saved as
metadata inside of a generated SQLite database.
Headers are only parsed if a SQLite database has been
opened, and if the full version of Worgle is being
compiled (aka no Worglite).

The function =is_header= is used to both check for
a valid header, as well as extract information from it
if it is indeed a header.

A valid header in org has one or more asterisks (*) followed
by at least one space. Anything following the spaces is
considered to be a the title.

The =is_header= function, if it is indeed a header, will
return the header level, as well as the title (stored as a
worgle string).
***** TODO Updating Section Position From Header
When a header has been extracted, the level is used to
update the current section position.
****** TODO The Section Position Data
The section data is stored as an array of unsigned integers.
The size of this array is determined by =WORGLE_MAXDEPTH=,
which limits how any subsections there can be. A decent
max depth size is 10.

The array in question begins at zero. When org-mode exports
data, the top-level number starts at 1 instead of 0. This
will be ignored for the sake of data consistency.

The current section depth relates to the position in the
array.
****** TODO Determining update position
The extracted header level is used to determine which
section number to increment. If the header number is greater
than the current section number, a new subsection is
created, and the position in the array is set to be zero.
If the value is less than or equal to the current section
number, that particular position is incremented by one.
***** TODO Saving Header Data
Once the section position has been updated, it can be saved
as metadata along with the header title to the database.
**** MODE_BEGINCODE
#+NAME:parse_modes
#+BEGIN_SRC c
MODE_BEGINCODE,
...
#+END_SRC
***** TODO Parsing Header Information
