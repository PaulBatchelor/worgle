...
#+BEGIN_SRC c
buf = calloc(1, size);
worgle_textbuf_init(&worg.buffers[0], buf, size);
/* worg.buf = buf; */
#+END_SRC
...
#+BEGIN_SRC c
int parse_file(worgle_d *worg, char *filename, int file);
#+END_SRC
...
#+NAME:worgle_init
#+BEGIN_SRC c
worgle_textbuf_zero(&worg->buffers[0]);
worg->curbuf = &worg->buffers[0]; /* hard coded for now */
#+END_SRC
...
#+NAME: worgle_free
#+BEGIN_SRC c
worgle_textbuf_free(&worg->buffers[0]);
#+END_SRC
**** Current Block
