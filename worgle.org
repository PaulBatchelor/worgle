...
appended segment is saved. This id, if a positive value,
would be the end segment of the previous block. This
information can be used later by worgmap to help extrapolate
which segments belong to which blocks. This is how the
variable gets the name =prev_last_seg=.
