...
int wmp_blkref_codeblock(wmp_core *c,
                         ...
                         wmp_segment *segs,
                         int *nsegs);
void wmp_blkref_codeblock_free(wmp_core *c, wmp_segment *segs);
#+END_SRC
...
the next block reference (if it exists).
These will be needed later on. These can be retrieved using
