...
# SELECT max(id) from resources where program == prog;
...
This ID value can be used to look up the next item.
...
Supplied must be the content to appended (duh), as well
